/**
 * odf-kit-service — Nextcloud Files action registration
 *
 * Registers "Export as ODT" in the Files context menu for markdown,
 * HTML, and plain text files. Uses @nextcloud/files v4 registerFileAction()
 * which writes to the scoped store read by Nextcloud 33's Files app.
 *
 * Built with esbuild (--external:@nextcloud/files) so it uses the copy
 * already loaded by Nextcloud rather than bundling its own.
 *
 * Loaded via AppAPI script registration — AppAPI proxies this file
 * from our Express server and injects it into every Nextcloud page.
 */

import { registerFileAction } from '@nextcloud/files'

// AppAPI proxy base path for our service
...
const ACTIONS: Array<{ id: string; mime: string }> = [
  ...
  registerFileAction({
    ...
    exec: async ({ node, currentUser }) => {
      try {
        ...
        return null  // null = success, no redirect needed
